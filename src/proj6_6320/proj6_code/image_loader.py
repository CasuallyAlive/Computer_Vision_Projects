...
class ImageLoader(data.Dataset):
  ...
  def load_imagepaths_with_labels(self, class_labels) -> List[Tuple[str, int]]:
    ...
    pth = self.curr_folder
    
    for class_label, idx in class_labels.items():
      
      class_path = os.path.join(pth, class_label) + '/*.jpg'
      image_dirs = glob.glob(class_path)
      img_paths.extend([(image_dir, idx) \
  ...
  def get_classes(self) -> dict:
    ...
    class_labels.sort()
    classes = {class_label : i for i, class_label in enumerate(class_labels)}
    # print(classes)
    ###########################################################################
    # Student code end
    ###########################################################################
